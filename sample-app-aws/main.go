...
func initAWS() *ec2metadata.EC2Metadata {
...
func onAWS() bool {
	var ec2MetadataService = initAWS()
	return ec2MetadataService.Available()
}

func getAWSMetadataValue(path string) string {
	var ec2MetadataService = initAWS()
	v, _ := ec2MetadataService.GetMetadata(fmt.Sprintf("%s", path))
	return v
}
...
func newInstance(db *gorm.DB) *Instance {
	...
	a := &assigner{}
	if onAWS() {
		i.Id = getAWSMetadataValue("/instance-id")
		i.Zone = getAWSMetadataValue("/placement/availability-zone")
		i.Name = getAWSMetadataValue("/local-hostname")
		i.Hostname = getAWSMetadataValue("/public-hostname")
		i.Project = getAWSMetadataValue("/services/partition")
		i.ContainerIP = GetOutboundIP().String()
		i.InternalIP = getAWSMetadataValue("/local-ipv4")
		i.ExternalIP = getAWSMetadataValue("/public-ipv4")
		i.Version = version
